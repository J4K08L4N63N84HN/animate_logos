...
Use for this the function insert_paths_ids.py prior to this function.
"""


def decompose_logos_in_folder(original_folder, target_folder):
    """ Decompose all SVGs in a folder.

    Args:
        original_folder (str): Path of folder containing all SVGs that need to be decomposed.
        target_folder (str): Path of folder containing all decomposed SVGs.
    """
    for file in os.listdir(original_folder):
        if file.endswith(".svg"):
            decompose_logo(original_folder + "/" + file, target_folder)


def decompose_logo(file, target_folder):
    """ Decompose one SVG.
    ...
    for i in range(num_elements):
        ...
        textfile = open(target_folder + filename + '_' + animation_id + '.svg', 'wb')
