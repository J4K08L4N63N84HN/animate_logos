...
from src.features.get_path_probabilities import get_path_probabilities
from src.preprocessing.insert_animation import create_animated_svg
...
def create_random_animations(folder_svg_dataset, animation_path_label="../../data/processed/path_animation_decision"
                             ...
                             split_df=True,
                             very_random=False):
    """ Create random animations. Animation vectors are saved in data/interim/logos_animated_random_information.

    Args:
        path_relevance_order:
        animation_path_label:
        folder_svg_dataset (str): Path of folder containing all SVGs.
    ...
    """
    Path("../../data/interim/logos_animated_random_information").mkdir(parents=True, exist_ok=True)
    if split_df:
        create_multiple_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order)
    else:
        return create_one_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order)


def create_multiple_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order):
    ...
    """
    for file in os.listdir(folder_svg_dataset):
        if file.endswith(".svg"):
            df = pd.DataFrame.from_records(
                _create_multiple_df(folder_svg_dataset, file, nb_animations, very_random, animation_path_label,
                                    path_relevance_order))
            output = open(f"../../data/interim/logos_animated_random_information"
                          f"/{file.replace('.svg', '')}_animation_vectors.pkl", 'wb')
            pickle.dump(df, output)
            output.close()


def _create_multiple_df(folder_svg_dataset, file, nb_animations, very_random, animation_path_label, path_relevance_order):
...
def create_one_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order):
    ...
    """
    df = pd.DataFrame.from_records(
        _create_one_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order))

    date_time = datetime.now().strftime('%H%M')
    output = open(f"../../data/interim/logos_animated_random_information/{date_time}_animation_vectors.pkl", 'wb')
    pickle.dump(df, output)
    output.close()
...
def _create_one_df(folder_svg_dataset, nb_animations, very_random, animation_path_label, path_relevance_order):
    for file in os.listdir(folder_svg_dataset):
        if file.endswith(".svg"):
            relevant_animation_ids = get_path_relevance(file.replace('.svg', ''), pkl_file=path_relevance_order)
            path_probs = get_path_probabilities(file.replace('.svg', ''), relevant_animation_ids,
